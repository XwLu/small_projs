                                  ...
                                  std::vector<c_int> *P_indptr)
{
  ...
  // const int num_of_nonzeros = num_of_variables;
    ...
    std::vector<c_float> *upper_bounds)
{
  ...
  const int n = static_cast<int>(num_of_knots_);
  const int num_of_variables = 3 * n;
  const int num_of_constraints = num_of_variables + 3 * (n - 1) + 3 + 2;
  ...
  upper_bounds->at(constraint_index) = 0;
  ++constraint_index;
...
bool OSQPProblem::Optimize(const int max_iter)
{
  ...
  {
    std::cout << "The solution from!!!!" << std::endl;
  }
